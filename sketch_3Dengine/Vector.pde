class Vector {
  ...
  Vector orthography(Vector vec) {
    return vec.multi(this.abs()*cosSimilarity(vec)/vec.abs());
  }
  float orthoSimilarity(Vector vec) {
    Vector orth = orthography(vec);
    if(cosSimilarity(vec)>=0){
      return orth.abs();
    }
    else
    {
  ...
  float angle(Vector vec) {
    return acos(cosSimilarity(vec));
  }
  float cosSimilarity(Vector vec) {
    return inner(vec)/(abs()*vec.abs());
  }
